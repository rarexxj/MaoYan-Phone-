    ...
    public class WeiXinController : Controller
    {
        ...
        public ActionResult LoginRedirt(string code, string state)
        {
            try
            {
                ...
                var user = _memberService.FindMemberById(oauth.MemberId)?.Simplified();
                if (user == null)
                    return Redirect("/html/signup.html?openId=" + openId);

                var userInfo = HttpUtility.UrlEncode(Convert.ToBase64String(Encoding.UTF8.GetBytes(user.ToJson())));
