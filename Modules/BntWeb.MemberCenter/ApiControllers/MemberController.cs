...
namespace BntWeb.MemberCenter.ApiControllers
{
    public class MemberController : BaseApiController
    {
        ...
        {
          // 优惠券
            var coupon =_currencyService.GetList<CouponUseRecord>(a=>!a.IsUsed&&a.MemberId==AuthorizedUser.Id).ToList(); ;
            //积分
            var integral = _walletService.GetWalletByMemberId(AuthorizedUser.Id, Wallet.Models.WalletType.Integral)?.Available;
        ...
        public ApiResult Update(string id, [FromBody]ModifyMemberModel member)
        {
            ...
            if (member.TastesType.ToString() !="")
                oldMember.TastesType = member.TastesType;
        ...
        public ApiResult BoundPhoneNumber(string id, [FromBody] BoundPhoneNumberModel member)
        {
            ...
            var user = _userManager.FindByName(AuthorizedUser.UserName);
