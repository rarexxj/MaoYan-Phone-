    ...
    _currencyService.GetSingleByConditon<OrderGoods>(o => o.OrderId == order.Id);
        ...
        public string AsyncReturn(HttpRequestBase request)
        {
            try
            {
                ...
                if (sPara.Count > 0)//判断是否有带返回参数
                {
                    Logger.Warning($"支付宝正在异步操作{request.Form["notify_id"]},{request.Form["sign"]}");
                    Logger.Warning($"支付宝正在异步操作-Partner：{config.Partner}");
                    Logger.Warning($"支付宝正在异步操作-SellerId：{config.SellerId}");
                    Logger.Warning($"支付宝正在异步操作-Md5Key：{config.Md5Key}");
                    Logger.Warning($"支付宝正在异步操作-SignType：{config.SignType}"); 
                    Logger.Warning($"支付宝正在异步操作-InputCharset：{config.InputCharset}");
                    Logger.Warning($"支付宝正在异步操作-AlipayPublicKey：{config.AlipayPublicKey.Trim()}");
                    ...
                    if (verifyResult)//验证成功
                    {
                        Logger.Warning($"支付宝正在异步操作-验证成功");
                        Logger.Warning($"支付宝正在异步操作-out_trade_no={request.Form["out_trade_no"]}，trade_no={request.Form["trade_no"]}，trade_status={request.Form["trade_status"]}");
                        /////////////////////////////////////////////////////////////////////////////////////////////////////////////
                        //请在这里加上商户的业务逻辑程序代码
            ...
            {
                Logger.Warning("支付宝回调异常："+ex.Message);
                return "fail";
            }
