...
namespace BntWeb.PaymentProcess.Controllers
{
    public class WeiXinController : Controller
    {
        ...
        public ActionResult ScanPay(PayViewModel payModel)
        {
            ...
            try
            {
                ...
                ViewBag.PayFee = order.PayFee;
                ViewBag.UrlImage = paymentDispatcher.WebPay(subject, body, notifyUrl, payModel.ReturnUrl, payLog, payment);
                ViewBag.SourceType = order.SourceType;
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult CheckPayStatus(Guid orderId)
        {
            var result = new DataJsonResult();

            var order = _currencyService.GetSingleById<Order>(orderId);

            if (order != null && order.OrderStatus == OrderStatus.WaitingForDelivery &&
                order.PayStatus == PayStatus.Paid)
            {
                return Json(result);
            }
            result.ErrorMessage = "未支付";
