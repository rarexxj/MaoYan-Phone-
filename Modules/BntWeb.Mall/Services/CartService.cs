...
namespace BntWeb.Mall.Services
{
    public class CartService:ICartService
    {
        ...
        public List<Cart> GetList(string memberId)
        {
            ...
            {
                var query =
                    dbContext.Carts.Where(me => me.MemberId == memberId )
        ...
        public void DeleteMemberCartInvalidGoods(string memberId)
        {
            using (var dbContext = new MallDbContext())
            {
                ...
                {
                    var singleGoods = dbContext.SingleGoods.FirstOrDefault(g => g.Id.Equals(cartInfo.SingleGoodsId));
                    if (singleGoods == null || singleGoods.Stock < 1)
