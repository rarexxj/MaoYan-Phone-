    ...
    public class GoodsService : IGoodsService
    {
        ...
        public Goods LoadFullGoods(Guid goodsId)
        {
            using (var dbContext = new MallDbContext())
            {
                ...
                goods.MainImage = _storageFileService.GetFiles(goodsId, MallModule.Key, "MainImage").FirstOrDefault()?.Simplified();
        ...
        /// <param name="totalCount"></param>
        /// <returns></returns>
        public List<Goods> GetHotGoodsByCategory(Guid? categoryId, string keyword, GoodsSortType sortType, int pageIndex, int pageSize, out int totalCount)
        {
            using (var dbContext = new MallDbContext())
            {
                var query = dbContext.Goods.Where(me => me.Status > 0 && me.IsHot);
                if (categoryId != null && categoryId != Guid.Empty)
                    query = query.Where(me => me.CategoryId == categoryId);
                if (!string.IsNullOrWhiteSpace(keyword))
                    query = query.Where(me => me.Name.Contains(keyword));
        ...
        public List<GoodsView> GetGoodsByCategory(Guid? categoryId, string keyword, List<Guid> brands, List<AttributeModel> others, decimal minPrice, decimal maxPrice, GoodsSortType sortType, int pageIndex, int pageSize, out int totalCount)
        {
            using (var dbContext = new MallDbContext())
            {
                ...
                totalCount = query.DistinctBy(x => x.GoodsView.Id).Count();
        ...
        public bool AddGoodsCollection(Guid goodsId, string memberId)
        {
            using (var dbContext = new MallDbContext())
            {
                var goodsModel = dbContext.Goods.FirstOrDefault(x => x.Id == goodsId);
                if (goodsModel == null)
                {
                ...
                var modelCollection =
                    dbContext.Markups.FirstOrDefault(
                        x => x.SourceId == goodsId && x.MemberId == memberId && x.MarkupType == MarkupType.Collect);
                if (modelCollection != null)
                {
                    throw new Exception("您已收藏！");
                }

                var model = new Markup
                {
                    Id = Guid.NewGuid(),
                    CreateTime = DateTime.Now,
                    MarkupType = MarkupType.Collect,
                    MemberId = memberId,
                    ModuleKey = MallModule.Key,
                    ModuleName = MallModule.Area,
                    SourceId = goodsId,
                    SourceType = ""
                };
                dbContext.Markups.Add(model);


                return dbContext.SaveChanges() > 0;
            }
        }
        public bool DelGoodsCollection(Guid goodsId, string memberId)
        {
            using (var dbContext = new MallDbContext())
            {
                var goodsModel = dbContext.Goods.FirstOrDefault(x => x.Id == goodsId);
                ...
                dbContext.Markups.Remove(modelCollection);
        ...
        //获取商品收藏数量
        public int GetCollectNumsByGoodsId(Guid goodsId)
        {
            using (var dbContext = new MallDbContext())
            {
               return dbContext.Markups.Count(x => x.SourceId == goodsId && x.MarkupType == MarkupType.Collect);
            }
        }
            ...
            using (var dbContext = new MallDbContext())
            {
             var query = (from e in dbContext.Evaluates
                             join s in dbContext.SingleGoods
                                   ...
                                   //on new { id = e.ExtentsionId, id1 = e.ExtentsionId }
                                   //equals new { id = s.Id, id1 = s.GoodsId }
                          where
                                 e.SourceType.Equals("Order") && e.ModuleKey.Equals(OrderProcessModule.Key, StringComparison.OrdinalIgnoreCase) &&
                                s.GoodsId == goodsId || e.GoodId==goodsId
       ...
       /// <param name="totalCount"></param>
       /// <returns></returns>
        public List<Evaluate.Models.Evaluate> GetGoodsEvalListByPage(Guid goodsId, int pageIndex, int pageSize, out int totalCount)
        {
            using (var dbContext = new MallDbContext())
            {
                var query = (from e in dbContext.Evaluates
                             join s in dbContext.Goods
                                   on e.ExtentsionId equals s.Id
                             //on new { id = e.ExtentsionId, id1 = e.ExtentsionId }
                             //equals new { id = s.Id, id1 = s.GoodsId }
                             where
                                    e.SourceType.Equals("Order") && e.ModuleKey.Equals(OrderProcessModule.Key, StringComparison.OrdinalIgnoreCase) &&
                                   s.Id == goodsId || e.GoodId == goodsId
                             select e).OrderByDescending(x => x.CreateTime);
        ...
        public List<Evaluate.Models.Evaluate> GetGoodsEvaluatesList(Guid goodsId)
        {
            using (var dbContext = new MallDbContext())
            {
                var query = (from e in dbContext.Evaluates
        ...
        /// </summary>
        /// <returns></returns>
        public List<Goods> GetOptionalGoods()
        {
            //自选商品
            using (var dbContext = new MallDbContext())
            {
                var optionalGoods = dbContext.Goods.Where(a => a.SpecialType == SpecialType.Optional && a.Status == GoodsStatus.InSale);

                foreach (var item in optionalGoods)
                {
                    item.MainImage =
                        _storageFileService.GetFiles(item.Id, MallModule.Key, "MainImage")
                            .FirstOrDefault()?
                            .Simplified();

                }
                return optionalGoods.ToList();
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public List<Goods> GePurchaseGoods()
        {
            //加价购商品
            using (var dbContext = new MallDbContext())
            {
                var purchaseGoods = dbContext.Goods.Where(a => a.SpecialType == SpecialType.PurchasePrice && a.Status != GoodsStatus.Delete);
        ...
        /// <returns></returns>
        public List<Goods> GeGoodPurchaseGoods(Guid goodId)
        {
            //加价购商品
            //选择该商品对应的加价购商品
            using(var dbContext=new MallDbContext())
            {
                var otionalIds = dbContext.Goods.Where(me => me.Id == goodId).Select(me=>me.RelationOpt);
                var purchaseGoods = dbContext.Goods.Where(a => otionalIds.Contains(a.Id.ToString())).ToList();
