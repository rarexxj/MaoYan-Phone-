...
namespace BntWeb.MemberBase.Services
{
    public class MemberService : IMemberService
    {
        ...
        {
            return _userManager.Users.FirstOrDefault(u => u.PhoneNumber == phone && u.UserType == UserType.Member);
        }
        ...
        /// <returns></returns>
        public User FindUserByNameOrPhone(string phoneNumber)
        {
            return _userManager.Users.FirstOrDefault(u => u.PhoneNumber == phoneNumber) ?? _userContainer.FindUser(phoneNumber);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public string GetMaxMemberCard()
        {
            using (var dbContext = new MemberDbContext())
            {
                var maxCard = dbContext.MemberExtensions.Max(x=>x.MemberCard);
                if (!string.IsNullOrEmpty(maxCard))
                    maxCard= maxCard.Substring(2);
                return maxCard;
            }
        }

        public int SaveMemberExtension(List<MemberExtension> member)
        {
            int count = 0;
            using (MemberDbContext dbContext = new MemberDbContext())
            {
                dbContext.MemberExtensions.AddRange(member);
