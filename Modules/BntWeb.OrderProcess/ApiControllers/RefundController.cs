...
namespace BntWeb.OrderProcess.ApiControllers
{
    public class RefundController : BaseApiController
    {
        ...
        public ApiResult ApplyRefund([FromBody]RefundApplyModel apply)
        {
            ...
            //        x => x.OrderId == apply.OrderId && x.SingleGoodsId == apply.SingleGoodsId);
            //var good =
            //    _currencyService.GetSingleByConditon<OrderGoods>(
            //        x => x.OrderId == apply.OrderId && x.SingleGoodsId == apply.GoodId);
            //var singleGoods = single ?? good;
            //if (singleGoods == null)
            ...
            //找到订单对应的商品 判断是否退款
            decimal maxRefundAmount = 0;
            var orderGood = _currencyService.GetList<OrderGoods>(a => a.OrderId == apply.OrderId);
            foreach (var item in orderGood)
            {
                if(item.RefundStatus!= OrderRefundStatus.NoRefund)
                    throw new WebApiInnerException("0007", "已申请退款不可重复申请");
                 maxRefundAmount+= (item.Price * item.Quantity);
            }

            maxRefundAmount -=order.CouponMoney;
            if (apply.RefundAmount > maxRefundAmount)
                  throw new WebApiInnerException("0008", "退款金额不能大于实付金额");
