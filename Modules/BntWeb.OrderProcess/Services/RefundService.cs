    ...
    public class RefundService : IRefundService
    {
        ...
        public void CreateOrderRefund(OrderRefund refund)
        {
            using (var dbContext = new OrderProcessDbContext())
            {
                ...
                //var orderGood = dbContext.OrderGoods.Where(x => x.OrderId == refund.OrderId);
                //foreach (var item in orderGood)
                //{
                //    item.RefundStatus = OrderRefundStatus.Refunding;
                //}
        ...
        public void AuditRefund(OrderRefund refund)
        {
            ...
            { bool orderGoodTrue = true;
                ...
                //退款订单商品
                var goods = _currencyService.GetList<OrderGoods>(x => x.OrderId == refund.OrderId).ToList();
                //审核通过
                                    ...
                                    foreach (var item in goods)
                                    {
                                        var goodf =
                                            _currencyService.GetSingleByConditon<OrderGoods>(
                                                a => a.GoodsId == item.GoodsId);
                                    goodf.RefundStatus= OrderRefundStatus.Refunded;
                                        if (_currencyService.Update(goodf))
                                        {
                                            orderGoodTrue = true;
                                        }
                                        else
                ...
                else
                {
                    ...
                    if (goods.Count != 0)
                    {
                        foreach (var item in goods)
                        {
                            item.RefundStatus = OrderRefundStatus.Refunded;
                        }
                    }
                    order.RefundStatus = GetOrderRefundStatus(refund, RefundStatus.Applying);
                }
                    ...
                    if (_currencyService.Update(refund) && orderGoodTrue && _currencyService.Update(order) && _currencyService.Create(orderAction))
                        //提交
                        scope.Complete();
