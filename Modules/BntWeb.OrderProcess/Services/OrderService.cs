...
namespace BntWeb.OrderProcess.Services
{
    public class OrderService : IOrderService
    {
        ...
        public Order Load(Guid orderId)
        {
            using (var dbContext = new OrderProcessDbContext())
            {
                ...
                if (order != null)
                {
                    ...
                    {
                        goods.GoodsImage = _storageFileService.GetFiles(goods.Id, OrderProcessModule.Key, "GoodsImage").FirstOrDefault()?.Simplified();
                    }
                }
        ...
        /// <param name="orderStatus">订单状态：-1已删除；0待付款；1待发货；2待收货；3已完成；4已关闭</param>
        /// <param name="payStatus">付款状态：0未付款；1付款中；2已付款</param>
        /// <param name="shippingStatus">0未发货；1已发货</param>
        /// <param name="evaluateStatus">0未评价；1已评价；2已回复</param>
                ...
                if (order != null)
                {
                   ...
                   //订单的物流、付款、评价状态非必传
                    ...
                    dbContext.OrderActions.Add(new OrderAction
                    {
                        ...
                        ShippingStatus = order.ShippingStatus,
                        EvaluateStatus = order.EvaluateStatus,
                        //表示currentUser不为空时为Id 为空时为Guid.Empty.ToString()
                        UserId = currentUser?.Id ?? Guid.Empty.ToString(),
                        UserName = currentUser?.UserName ?? "系统"
                    });
        ...
        public List<Order> LoadByPage(string memberId, out int totalCount, OrderStatus? orderStatus = null, PayStatus? payStatus = null,
            ShippingStatus? shippingStatus = null, EvaluateStatus? evaluateStatus = null, OrderRefundStatus? refundStatus = null, string keywords = "", 
            int pageIndex = 1, int pageSize = 10, OrderStatus? extentOrderStatus = null, OrderRefundStatus? extentRefundStatus = null)
        {
            using (var dbContext = new OrderProcessDbContext())
            {
                ...
                var query =
                    dbContext.Orders.Where(o => o.MemberId.Equals(memberId, StringComparison.OrdinalIgnoreCase) && 
                    o.OrderStatus != OrderStatus.Deleted && (orderStatus == null || (o.OrderStatus == orderStatus.Value || (extentOrderStatus != null && o.OrderStatus == extentOrderStatus.Value)))
                    && (payStatus == null || o.PayStatus == payStatus.Value) &&
                    (shippingStatus == null || o.ShippingStatus == shippingStatus.Value) 
                    && (evaluateStatus == null || o.EvaluateStatus == evaluateStatus.Value)
                    && (refundStatus == null || (o.RefundStatus == refundStatus.Value || (extentRefundStatus != null 
                    && o.RefundStatus == extentRefundStatus.Value))) &&
                    (noKeywords || o.OrderNo.Contains(keywords))).Include(o => o.OrderGoods).OrderByDescending(o => o.CreateTime);
            ...
            using (var dbContext = new OrderProcessDbContext())
            {
                var query = from g in dbContext.OrderGoods
                            ...
                            select new EvaluateDetailsModel
                            {
                                ...
                                Price = g.Price,
                                Quantity = g.Quantity,
                                GoodTasteScore = e.GoodTasteScore,
                                Content = e.Content,
        ...
        public List<EvaluateDetailsModel> LoadOrderEvaluateList(Guid orderId,string memberId)
        {
            using (var dbContext = new OrderProcessDbContext())
            {
                var query = from g in dbContext.OrderGoods
                            join e in dbContext.Evaluates on g.Id equals e.SourceId
                            where g.OrderId == orderId && e.SourceType.Equals("Order") && e.MemberId== memberId
                            select new EvaluateDetailsModel
                            {
                                Id=e.Id,
                                SingleGoodsId = g.SingleGoodsId,
                                ...
                                Quantity = g.Quantity,
                                GoodTasteScore = e.GoodTasteScore,
                                FreshMaterialScore=e.FreshMaterialScore,
                                LogisticsScore = e.LogisticsScore,
                                DesMatchScore = e.DesMatchScore,
                                Content = e.Content,
                                IsAnonymity = e.IsAnonymity,
                                MemberName = e.MemberName,
                                EvaluateTime = e.CreateTime,
        ...
        /// <returns></returns>
        public StateOrderCount StateOrderCount(string memberId)
        {
            using (var dbContext = new OrderProcessDbContext())
            {
                var state = new StateOrderCount();
                state.PendingPaymentCount = dbContext.Orders.Count(me => me.MemberId == memberId && me.OrderStatus == OrderStatus.PendingPayment);
                state.WaitingForReceivingCount = dbContext.Orders.Count(me => me.MemberId == memberId && me.OrderStatus == OrderStatus.WaitingForReceiving);
                state.WaitingEvaluateCount = dbContext.Orders.Count(me => me.MemberId == memberId && me.OrderStatus == OrderStatus.Completed && me.EvaluateStatus == EvaluateStatus.NotEvaluated);
                state.RefundCount = dbContext.Orders.Count(me => me.MemberId == memberId && me.RefundStatus != OrderRefundStatus.NoRefund);
                state.TotalCount = dbContext.Orders.Count(me => me.MemberId == memberId && me.OrderStatus != OrderStatus.Deleted);
                return state;
            }
        }
