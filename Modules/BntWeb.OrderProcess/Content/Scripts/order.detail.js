jQuery(function ($) {
    ...
    //货到付款确守收货并收款记录
    $('.order-actions').on("click", ".c", function (e) {
        var action = $(this).data("action");
        bntToolkit.confirm("确定要确认收货吗", function () {
            bntToolkit.post(_.find(urls, function (o) { return o.name == action; }).url, { orderId: orderId }, function (result) {
                if (result.Success) {
                    location.reload();
                } else {
                    bntToolkit.error(result.ErrorMessage);
